...
columna_C = [celda.value for celda in sheetRollup['C'] if celda.value is not None]

# Leer los datos de la columna C en 'Hoja2'
columna_D = [celda.value for celda in sheetRecs['A'] if celda.value is not None]


# Combinar los elementos de las columnas A y B como conjuntos (pares)
pares_AB = [(a, b, c ) for a, b, c in zip(columna_A, columna_B, columna_C)]

# Generar las combinaciones posibles entre los pares (A, B) y cada elemento de la columna C
combinaciones = [(a, b, c, d) for (a, b, c) in pares_AB for d in columna_D]
